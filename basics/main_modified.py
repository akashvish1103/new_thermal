# Replace forehead rectangular ROI with the polygon ROI for forehead temperature extraction
# Rest functionality is same as merge2.py file, but here we are using the polygon ROI for forehead instead of rectangular ROI.

import cv2
import mediapipe as mp
import numpy as np
import matplotlib.pyplot as plt
import utilities_modified as ut
...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        for face_landmarks in results.multi_face_landmarks:          # iterating over all the faces 
            ...
            forehead_points = ut.get_forehead_coordinates(
                frame,
                face_landmarks,
                flag
            )
            ...
            polygon_forehead = np.array(forehead_points,dtype=np.int32)
            mask_forehead = np.zeros(grey.shape,dtype=np.uint8)
            cv2.fillPoly(mask_forehead,[polygon_forehead],255)
            forehead_pixels = grey[mask_forehead==255]
            if forehead_pixels.size>0:
                pixel_percentage=0.10
                forehead_pixels=np.sort(forehead_pixels)[::-1]
                forehead_pixels=forehead_pixels[:max(1,int(len(forehead_pixels)*pixel_percentage))]
                forehead_mean_pixel=np.mean(forehead_pixels)
                forehead_temperature=pixel_to_temperature(forehead_mean_pixel)
                forehead_temperatures.append(forehead_temperature)
