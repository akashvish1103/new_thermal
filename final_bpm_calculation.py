...
import utility_linear_mapping_cld as ulm          # <-- NEW: pixel -> temperature calibration utilities (modified cld version)
...
video_path = r"D:\Tihar_thermal_data_Input\Sabarmati_sample_data\62_2026-07-13\02_Psychometric_Tests\62_HDRS_Thermal_30_40.mpg"

# ─────────────────────────────────────────────────────────────────────────────
# CALIBRATION — pixel-to-temperature mapping, computed once per video
# ─────────────────────────────────────────────────────────────────────────────
# Reads the legend/color-palette column from the video's first frame and the
# min/max temperature encoded in the filename, then derives a linear mapping
# specific to THIS video (replaces the old fixed m, b constants).

legend_pixel_column = ulm.get_one_pixel_column(video_path)

min_value, max_value, array_first_element, array_last_element = \
    ulm.get_first_last_element_from_OnePixelColumn_AND_min_max_from_filename(
        legend_pixel_column, video_path
    )


def get_temp_from_pixel(pixel_value):
    """Linear mapping: pixel intensity -> temperature (deg C), calibrated per-video."""
