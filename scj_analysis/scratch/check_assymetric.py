...
# CORRECT PAIRING: this video matches 44_Passive_Markers.csv (subject 44, full ~698s session)
video_path = r"D:\000_ofc_thermalData\sorted_data\48\48_HDRS_Thermal_30_40.mpg" # <-- update to the real full path on your machine

LOG_PATH = os.path.splitext(video_path)[0] + "_roi_log.csv"

# ============================================================
# FFmpeg pre-conversion (fixes OpenCV silently truncating some
# .mpg/.mpeg files partway through -- their codec support for old
# MPEG-2 thermal exports is unreliable). We convert to a clean
# H.264 .mp4 in a temp file and process THAT, while calibration
# (which parses min/max temp from the filename) still reads the
# ORIGINAL file/path further down.
# ============================================================

FFMPEG_EXE = r"C:\ffmpeg-2026-08-20-git-7d77562d2a-essentials_build\bin\ffmpeg.exe"  # <-- update to your ffmpeg.exe path


def convert_to_mp4(input_video):
    """Convert input_video to a temporary MP4 via FFmpeg. Original file is
    never modified. Returns the path to the temporary MP4."""
    ...
    command = [
        ...
        "-c:v", "libx264", "-pix_fmt", "yuv420p", "-an", temp_mp4
    ]

    print("=" * 70)
    print("CONVERTING INPUT VIDEO USING FFMPEG")
    print(f"Input:  {input_video}")
    print(f"Temp:   {temp_mp4}")
    print("=" * 70)
    ...
    try:
        ...
        if os.path.exists(temp_mp4):
            os.remove(temp_mp4)
        raise RuntimeError(
            "FFmpeg was not found. Verify `ffmpeg -version` works and "
            "FFMPEG_EXE above points to the right executable."
        )
    except subprocess.CalledProcessError as e:
        if os.path.exists(temp_mp4):
            os.remove(temp_mp4)
        raise RuntimeError(f"FFmpeg conversion failed with exit code {e.returncode}.")

    if not os.path.isfile(temp_mp4):
        raise RuntimeError("FFmpeg finished, but the temporary MP4 was not created.")
...
if input_extension in [".mpg", ".mpeg", ".mpv"]:
    temporary_video = convert_to_mp4(video_path)
    processing_video_path = temporary_video
else:
    # .wmv / .mp4 / other formats OpenCV already handles natively -- process directly
...
# IMPORTANT: always reads the ORIGINAL video_path (not the FFmpeg
# temp file) because the min/max temp are parsed from the filename.
# ============================================================
...
if not cap.isOpened():
    ...
    raise RuntimeError(f"OpenCV could not open the video:\n{processing_video_path}")
...
print(f"Metadata frame count: {total_frames_metadata}  "
      f"(implied duration: {total_frames_metadata / fps:.2f}s -- sanity-check "
      f"this against the file's real duration before trusting time_sec)")
print("=" * 70)
...
try:
    ...
    # always runs -- even on crash, Ctrl+C, or early break -- so partial
    # logs are never lost and the temp FFmpeg file never lingers on disk
    log_file.close()
    cap.release()
    ...
    if temporary_video is not None:
        try:
            os.remove(temporary_video)
            print("Temporary converted MP4 deleted.")
        except OSError as e:
            print(f"WARNING: could not delete temporary MP4: {temporary_video}\n{e}")

print(f"\nDone. Processed {frame_number + 1} frames. ROI log saved to: {LOG_PATH}")